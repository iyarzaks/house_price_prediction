...
def create_neighbors_dict_and_data(test_split,total_size):
    ...
    all_data = pd.concat([x_test_with_probs, x_train_with_probs],ignore_index=True, axis=0)
    columns_for_mrf = ['long_un_norm','lat_un_norm',0,1,2,3,4,'price']
    ...
    write_to_pkl(neigh_dict, 'neighbors_dict_lr.pkl')
    write_to_pkl(all_data, 'single_potentials_all_nodes_lr.pkl')
    total = time.time() - s_time
    str(timedelta(seconds=total))


def create_pairwise_potentials():
    neigh_dict = read_from_pkl('neighbors_dict_lr.pkl')
    print (len(neigh_dict))
    all_data = read_from_pkl('single_potentials_all_nodes_lr.pkl')
    potentials = mrf.build_potentials(neigh_dict, all_data)
    write_to_pkl(potentials, 'potentials_dict_all_nodes_lr.pkl')


def main():
    create_neighbors_dict_and_data(test_split=0.25, total_size=21613)
